...
class AdvIRL(TorchBaseAlgorithm):
    ...
    def _do_policy_training(self, epoch):
        ...
        if self.mode == "gail" and self.dac == False:
            policy_batch["rewards"] = F.softplus(
                disc_logits, beta=-1
            )
        elif self.mode == "gail" and self.dac == True:
            probs = torch.sigmoid(disc_logits)
            # torch.log(probs + 1e-8) - torch.log(1 - probs + 1e-8)
            policy_batch["rewards"] = torch.log(probs + 1e-8) - torch.log(1 - probs + 1e-8)
        elif self.mode == "ddpm":
